...
def otaduck():
    try:
        online = requests.get("https://raw.githubusercontent.com/TA2KVC/Vakvak/main/OTA/duck.py")
...
def otacat():
    try:
        online1 = requests.get("https://raw.githubusercontent.com/TA2KVC/Vakvak/main/OTA/kedi.py")
...
def otadog():
    try:
        online2 = requests.get("https://raw.githubusercontent.com/TA2KVC/Vakvak/main/OTA/dog.py")
...
def otamix():
    try:
        online3 = requests.get("https://raw.githubusercontent.com/TA2KVC/Vakvak/main/OTA/cute.py")
...
def atam():
    try:
        ataf = requests.get("https://raw.githubusercontent.com/TA2KVC/Vakvak/main/OTA/atam.py")
        ataff = (ataf.text).splitlines()
        val1 = random.choice(ataff)
        ataf.close()
        line4 = requests.get("https://raw.githubusercontent.com/TA2KVC/Vakvak/main/OTA/ata.txt")
...
def accu():
    ...
    response = requests.get(f'http://dataservice.accuweather.com/currentconditions/v1/316938?apikey=xxxxxxxxxxxxxxxxxxxx&language=tr-tr&details=true', headers=headers)
...
def accukod(city):
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3'}
    kod = requests.get(f'http://dataservice.accuweather.com/locations/v1/cities/search?apikey=xxxxxxxxxxxxxxx&q={city}&language=tr-tr', headers=headers)
    koda = kod.json()[0]
    key = koda['Key']
    response = requests.get(f'http://dataservice.accuweather.com/currentconditions/v1/{key}?apikey=xxxxxxxxxxxxxxxxxxxxxx&language=tr-tr&details=true', headers=headers)
    data = response.json()[0]
    info = data['WeatherText']
    scak = str(data['Temperature']['Metric']['Value'])
    nem = str(data['RelativeHumidity'])
...
def yts():
    try:
        headers = {"Accept-Language": "tr-TR,tr;q=0.5"}
        ytstotal = requests.get("https://yts.mx/api/v2/list_movies.json")
        total = ytstotal.json()
        count= total['data']['movie_count']
        rndm = random.randint(1,count)
        movi = requests.get(f"https://yts.mx/api/v2/movie_details.json?movie_id={rndm}", headers=headers)
        movie = movi.json()
        title = movie['data']['movie']['title_long']
        poster = movie['data']['movie']['large_cover_image']
        ge = movie['data']['movie']['genres']
        gen = (', '.join(ge))
        return title,poster,gen
    except:
        return False
